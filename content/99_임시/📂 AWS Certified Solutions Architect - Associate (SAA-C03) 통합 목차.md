...
|   |   |   |
|---|---|---|
| 특징 | **표준 대기열 (Standard)** | **FIFO 대기열** | ❌ 오답이 되는 조건 |
|------|------|------|------|
| **순서 보장** | 최선 노력, 보장 안 함 | **정확히 FIFO 순서 보장** | "순서 중요" → Standard 오답 |
| **전달 횟수** | 최소 1회 (중복 가능) | **정확히 1회 (Exactly-once)** | "중복 제거 필요" → Standard 오답 |
| **처리량** | 무제한 | 초당 300~3,000건 (제한적) | "초고속 처리량 필요" → FIFO 오답 |
| **용도** | 일반 작업 부하 분산 | **결제, 주문 처리** (Q10, Q149, Q181) | 순서 무관한 대용량 처리 → FIFO 오답 |
...
1. SNS 단독 사용 시 구독자 서버가 다운되면 메시지는 어떻게 되는가? 어떻게 해결하는가?
2. "주문을 순서대로 처리하고 중복도 없어야 한다" → Standard가 오답인 이유는?
3. Visibility Timeout이 부족하면 어떤 문제가 생기는가? 어떻게 설정해야 하는가?
4. "하나의 이벤트를 여러 시스템이 동시에 받아 처리" → 어떤 아키텍처 패턴인가?
5. SQS vs SNS 중 메시지를 보존하는 서비스는? 이 차이가 설계에서 왜 중요한가?
...
| 기능 | 상세 내용 | 시험 포인트 (키워드) | ❌ 오답이 되는 조건 |
|------|------|------|------|
| **상태 관리** | 각 단계 성공/실패 기억 + 재시도 | 분산 애플리케이션 조율 (Q366, Q375) | 단순 1:1 Lambda 트리거 → EventBridge면 충분 |
| **수동 승인** | 중간에 사람이 승인해야 다음 단계 진행 | **"인적 검토(Manual Approval)"** 필요 시 | 자동화만 필요한 경우 (불필요한 복잡도) |
| **병렬 처리** | 여러 작업 동시 실행 + 결과 취합 | 데이터 처리 가속 (Q603) | - |
| **장기 실행** | 최대 1년까지 워크플로 유지 | Lambda 15분 한계 극복 | 단순 이벤트 라우팅만 필요 시 → EventBridge |
...
- **선택 기준**: 기존 앱이 **ActiveMQ**나 **RabbitMQ**를 사용 중인 경우 (Q111, Q138)
...
1. Lambda 15분 제한을 초과하는 복잡한 비즈니스 로직을 어떻게 처리하는가?
2. Step Functions에서 "인적 검토(Manual Approval)"가 필요한 시나리오를 설명하라.
3. AppFlow가 필요한 상황은? 직접 API 코딩 대비 장점은?
4. Amazon MQ를 SQS 대신 선택하는 유일한 이유는 무엇인가?
5. Step Functions의 Map State는 어떤 문제를 해결하는가?
...
> - "데이터를 **변환**해서 **S3/Redshift**에 바로 저장해야 함" → **KDF**
...
|   |   |   |
|---|---|---|
| 비교 항목 | **Amazon SQS** | **Amazon Kinesis (KDS)** | ❌ 오답이 되는 조건 |
|------|------|------|------|
| **모델** | Pull (메시지 가져가면 삭제) | 스트림 (여러 소비자 반복 읽기) | "여러 앱이 같은 데이터 반복 읽기" → SQS 오답 |
| **순서** | FIFO 설정 시에만 보장 | **파티션 키별 항상 순서 보장** | - |
| **Replay** | **불가** (소비 후 삭제) | **가능** (보존 기간 내 재처리) | "데이터 재처리/재생" → SQS 오답 |
| **용도** | 서비스 결합 해제, 작업 큐 | 빅데이터 수집, 클릭스트림, 로그 집계 | 단순 작업 분산 → Kinesis 오버스펙 |
| **처리량** | 무제한 (Standard) | 샤드 수에 따라 결정 | - |

> [!warning] SQS vs Kinesis 핵심 선택 기준
...
1. "여러 앱이 같은 데이터를 각자 읽어야 한다" → SQS가 오답인 이유와 올바른 선택은?
2. KDS vs KDF 선택 기준을 지연시간과 관리 난이도 기준으로 설명하면?
3. "데이터를 어제 처리 못했는데 다시 읽어야 한다(Replay)" → SQS가 불가한 이유는?
4. KDF에서 JSON → Parquet으로 변환하려면 → 어떻게 설정하는가?
5. Kinesis와 SQS 둘 다 데이터를 전달하는데 근본적인 모델 차이는?
...
- **S3 비용 절감**: 접근 빈도에 따라 **S3 Lifecycle 정책**으로 계층 이동. (Q23, Q126)
- **EC2 비용 절감**:
...
1. Cost Explorer와 AWS Budgets의 차이를 설명하라. 각각 언제 쓰는가?
2. Compute Savings Plans와 EC2 Instance Savings Plans의 차이는?
3. AWS Organizations 통합 빌링에서 Savings Plans 할인이 공유되는 방식을 설명하라.
4. Cost Anomaly Detection은 어떤 문제를 해결하는가? AWS Budgets와의 차이는?
5. 부서별 비용을 추적하려면 어떤 AWS 기능을 활용해야 하는가?
...
- **RTO/RPO**: SAA 시험에서 **가장 빠른 DR 성능**을 요구할 때 정답.
...
- **보안**: **AWS Backup Vault Lock**을 사용하여 백업 삭제 방지 (규정 준수).
...
1. DR 4대 전략을 비용 순서로 나열하고 각각의 RTO/RPO 특성을 설명하라.
2. Pilot Light와 Warm Standby의 결정적 차이는 무엇인가?
3. RPO와 RTO의 차이를 실제 시나리오를 들어 설명하라.
4. Aurora Global Database를 쓸 때와 Multi-AZ를 쓸 때의 차이는?
5. Route 53 Failover 라우팅에서 Primary/Secondary에 각각 무엇을 놓는가?
