...
## 10. 고도화: MSA 전환 및 컨테이너 오케스트레이션 (ECS Fargate)

기존 단일 Monolithic 아키텍처에서 발생하던 특정 도메인의 트래픽 몰림에 따른 전체 시스템 장애 위험성을 해소하고자, 시스템을 마이크로서비스 아키텍처(MSA)로 개편하고 AWS 컨테이너 환경(ECR + ECS Fargate)으로 전면 마이그레이션했습니다.

### 마이크로서비스 아키텍처(MSA) 재설계
- **도메인 주도 설계(DDD) 기반 분리**: 기존 단일 애플리케이션을 유저(User), 상품(Product), 타임딜 주문(Order), 결제(Payment) 도메인 단위로 완전히 분리했습니다. 각 마이크로서비스는 독립적인 데이터베이스를 가지며 서로의 장애에 영향을 받지 않도록 고립성을 확보했습니다.
- **이벤트 기반 아키텍처(EDA) 도입**: 서비스 간 강결합을 줄이기 위해 Apache Kafka를 활용한 비동기 메시지 큐 시스템을 도입했습니다. 타임딜 주문 생성 시 즉시 응답을 반환하고 결제 처리를 백그라운드로 전환하여 순간적인 트래픽 폭주에도 유연하게 대처합니다.

### AWS ECS Fargate 기반 인프라 마이그레이션
- **컨테이너 이미지 중앙 관리 (ECR)**: 분리된 4개의 마이크로서비스를 각각 도커(Docker) 컨테이너로 빌드하고, AWS ECR(Elastic Container Registry)을 활용해 안전하게 이미지를 저장 및 버전 관리하는 체계를 구축했습니다.
- **서버리스 컨테이너 오케스트레이션 (ECS Fargate)**: 기존 EC2 인스턴스 직접 관리 방식에서 벗어나, 인프라 프로비저닝 없이 컨테이너를 실행할 수 있는 AWS Fargate 기반 ECS(Elastic Container Service)로 전환했습니다.
- **오토 스케일링(Auto Scaling)**: 타임딜 이벤트 오픈과 같은 폭발적 트래픽 상황 시, CPU 및 메모리 사용량 지표를 기반으로 컨테이너 태스크(Task)가 순식간에 확장(Scale-out)될 수 있는 탄력적 인프라를 완성했습니다.
